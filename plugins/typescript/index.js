...
function modify(baseConfig, { target, dev }, webpack, userOptions = {}) {
  ...
  // Provide default config files if they don't exist in project
  ...
  const appTsLintPath = path.join(process.cwd(), 'tslint.json')
  try {
    fs.statSync(appTsLintPath)
  } catch(e) {
    fs.copyFileSync(
      require.resolve('./tslint.json'),
      appTsLintPath
    )
  }
